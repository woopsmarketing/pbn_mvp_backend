...
def send_welcome_email(user_email: str):
    ...
    try:
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #2563eb;">🎉 BacklinkVending에 오신 것을 환영합니다!</h2>
...
def send_backlink_completion_email(
    ...
    try:
        ...
        result = email_service.send_email(
            to_email=user_email,
            subject=f"[BacklinkVending] 백링크 구축이 완료되었습니다! - {order_id}",
            html_content=html_content,
        )
...
def send_admin_failure_alert(
    ...
    try:
        ...
        result = email_service.send_email(
            ...
            subject=f"[BacklinkVending 관리자] 백링크 구축 실패 - {order_id}",
            html_content=html_content,
        )
...
def send_backlink_report_email(user_email: str, backlinks: List[Dict[str, Any]]):
    ...
    try:
        ...
        result = email_service.send_email(
            to_email=user_email,
            subject=f"[BacklinkVending] 백링크 보고서 ({total_backlinks}개)",
            html_content=html_content,
        )
...
def send_event_notification_email(user_email: str, event_type: str, event_data: dict):
    ...
    try:
        ...
        if event_type == "promotion":
            subject = "[BacklinkVending] 🎁 특별 프로모션 안내"
            content = "새로운 프로모션이 시작되었습니다!"
        elif event_type == "system_update":
            subject = "[BacklinkVending] 🔧 시스템 업데이트 안내"
            content = "시스템이 업데이트되었습니다."
        else:
            subject = f"[BacklinkVending] {event_type} 알림"
